"""
Chikusei Dataset - Hyperspectral imagery for land cover classification.
Dataset from https://www.sal.t.u-tokyo.ac.jp/hyperdata/
"""
...
@DatasetManager.register_dataset('chikusei')
class ChikuseiDataset(BaseDataset):
    """
    Chikusei hyperspectral dataset for land cover classification.

    Contains 19 land cover classes from airborne hyperspectral imagery
    128 spectral bands
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        """
        Initialize Chikusei dataset.

        Args:
            data_dir: Path to Chikusei folder containing extracted windows
    ...
    def _load_data(self):
        """Load Chikusei dataset from pre-extracted windows."""
        if self.verbose:
            print(f"Loading Chikusei dataset")

        # Load data files
        windows_file = self.data_dir / 'chikusei_windows_data.npy'
        labels_file = self.data_dir / 'chikusei_windows_labels.npy'

        if not windows_file.exists() or not labels_file.exists():
            raise FileNotFoundError(
                f"Data files not found in {self.data_dir}\n"
                f"Expected files: chikusei_windows_data.npy, chikusei_windows_labels.npy"
            )
        ...
        # Setup classes
        self.classes = [f"LandCover_{label}" for label in unique_labels]
